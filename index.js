  ...
  .createServer(function (req, res) {
    ...
    if (url == "/api/sc/player/details") {
      req.on("data", (data) => {
        ...
        res.end(
          fs.readFileSync(`${__dirname}/resources/data/player-details.json`)
        );
      });
      return;
      ...
      res.writeHead(200, { "Content-Type": "text/html" });
      res.end(
        fs.readFileSync(`${__dirname}/resources/views/privacy-policy.html`)
      );
      return;
    } else if (bbsIndx > -1) {
      ...
      url =
        url.substring(0, challengSportIndex) +
        url.substring(challengSportIndex + 17);
      target = "http://localhost:5053";
      req.url = url + ".php";
    } else if (predictionSportIndx > -1) {
      url =
        url.substring(0, predictionSportIndx) +
        url.substring(predictionSportIndx + 16);
      target = "http://localhost:5057";
      req.url = url;
    } else if (sportCardsIndx > -1) {
      url =
        url.substring(0, sportCardsIndx) + url.substring(sportCardsIndx + 15);
